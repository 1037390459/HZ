...
@interface PsdSetViewController (){
    // 提示lab
...
@implementation PsdSetViewController
...
- (void)creatSubviews{
    ...
    alarmLabel.text = [NSString stringWithFormat:@"您注册的手机号码为%@,请设置密码",_accountStr];
...
- (void)okButtonClick{
    [MBProgressHUD showHUDAddedTo:self.view animated:true];
       ...
       [parameters setValue:@(self.countryId) forKey:@"city_id"];
       [parameters setValue:pasText1.text forKey:@"pwd"];
       [parameters setValue:self.code forKey:@"code"];
          [[self class]POST:[NSString stringWithFormat:@"%@/index.php/login/registered",HOST_URL] parameters:parameters progress:nil completionHandler:^(id responseObj, NSError *error) {
              @strongify(self);
